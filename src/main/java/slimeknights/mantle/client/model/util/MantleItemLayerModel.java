...
public class MantleItemLayerModel implements IUnbakedGeometry<MantleItemLayerModel> {
  ...
  public static BakedQuad getQuadForGui(int color, int tint, TextureAtlasSprite sprite, Transformation transform, int emissivity) {
    // setup quad builder
    List<BakedQuad> builder = new ArrayList<>(1);
    QuadBakingVertexConsumer quadBuilder = new QuadBakingVertexConsumer();
    // common settings
    quadBuilder.setSprite(sprite);
    quadBuilder.setTintIndex(tint);
   ...
   */
  private static void buildSideQuad(List<BakedQuad> quads, QuadBakingVertexConsumer builder, VertexConsumer consumer, Direction side, int color, TextureAtlasSprite sprite, int u, int v, int size, int luminosity) {
    ...
    float v0 = 1f - y0 - dy;
    float v1 = 1f - y1 - dy;
    buildQuad(quads, builder, consumer, (side.getAxis() == Axis.Y ? side.getOpposite() : side),
      color, luminosity,
   ...
   * @param luminosity Extra light to add to the quad between 0 and 15
   */
  public static void buildQuad(List<BakedQuad> quads, QuadBakingVertexConsumer builder, VertexConsumer consumer, Direction side, int color, int luminosity,
