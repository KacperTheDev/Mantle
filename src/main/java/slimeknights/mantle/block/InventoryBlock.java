...
public abstract class InventoryBlock extends Block implements EntityBlock {
  ...
  protected boolean openGui(Player player, Level world, BlockPos pos) {
    if (!world.isClientSide()) {
      ...
      if (container != null && player instanceof ServerPlayer serverPlayer) {
        serverPlayer.openMenu(container, (RegistryFriendlyByteBuf buffer) -> buffer.writeBlockPos(pos));
  ...
  public void onRemove(BlockState state, Level worldIn, BlockPos pos, BlockState newState, boolean isMoving) {
    if (state.getBlock() != newState.getBlock()) {
      ...
      if (te != null) {
        IItemHandler inventory = Capabilities.ItemHandler.BLOCK.getCapability(worldIn, pos, state, te, null);
        if (inventory != null) {
