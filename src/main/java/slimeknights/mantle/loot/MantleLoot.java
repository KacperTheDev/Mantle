...
public class MantleLoot {
  ...
  public static void registerGlobalLootModifiers(final RegisterEvent event) {
    ...
    if (key == NeoForgeRegistries.Keys.GLOBAL_LOOT_MODIFIER_SERIALIZERS) {
      ...
      TAG_EMPTY = Registry.register(BuiltInRegistries.LOOT_CONDITION_TYPE, TagEmptyCondition.SERIALIZER.getID(), new LootItemConditionType(TagEmptyCondition.SERIALIZER.codec()));
      TAG_FILLED = Registry.register(BuiltInRegistries.LOOT_CONDITION_TYPE, TagFilledCondition.SERIALIZER.getID(), new LootItemConditionType(TagFilledCondition.SERIALIZER.codec()));
   ...
   */
  private static <T extends net.minecraft.world.level.storage.loot.functions.LootItemFunction> LootItemFunctionType<T> registerFunction(String name, MapCodec<T> codec) {
