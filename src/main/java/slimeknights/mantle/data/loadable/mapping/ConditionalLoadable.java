...
public record ConditionalLoadable<T extends IHaveLoader>(GenericLoaderRegistry<T> registry, @Nullable T defaultIfFalse) implements RecordLoadable<T> {
  ...
  public T deserialize(JsonObject json, TypedMap context) {
    ...
    if (!json.has("conditions") || ICondition.LIST_CODEC.parse(JsonOps.INSTANCE, json.get("conditions")).getOrThrow(JsonParseException::new).stream().allMatch(condition -> condition.test(conditionContext))) {
  ...
  public void serialize(T object, JsonObject json) {
    ...
    json.add("conditions", ICondition.LIST_CODEC.encodeStart(JsonOps.INSTANCE, Arrays.asList(conditional.conditions())).getOrThrow(JsonParseException::new));
